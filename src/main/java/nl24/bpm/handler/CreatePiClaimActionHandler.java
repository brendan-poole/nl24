...
@Service
public class CreatePiClaimActionHandler extends ActionHandler {
	...
	public CreatePiClaimActionHandler() {
		LOG.debug(this);
	}
	...
	public List<Transition> performAction(ProcessInstance processInstance,  List<String> messages) throws BpmException {
		...
		c.merge();
		messages.add("Created PI claim for "+c.getClaim().getPassenger().getContact()+" with claim ref "+c.getReference());
